...
Вычисляем (угол атаки 15°, 70 км/час, 2.5 м² крылья):  
```
...
Вычисляем (угол атаки 9°, 100 км/час, 2.5 м² крылья):  
```
...
Вычисляем (угол атаки 0°, 200 км/час, 2.5 м² крылья):  
```
...
Вычисляем (угол атаки 15°, 70 км/час, 15 м² контур левитации):  
```
1.2*((1.2*(70/3.6)^2)/2)*15=4064 ньтонов (414 кгс)
```

С капелькой магии пегас может поднять своё тело и ещё 350 килограмм груза. При этом система пегас-контур ведёт себя как самолёт типа «летающее крыло», со всеми своими достоинствами и недостатками. А дальше вмешивается аэродинамическое сопротивление и опасность сваливания, поэтому грузоподъёмность приходится ограничить чуть ли не вдвое от предельного числа.

Вычисляем (угол атаки 0°, 200 км/час, 15 м² планёр):  
```
...
Полуоси r=0.15 и R=0.25 метра: `3.14159265*0.25*0.15=0.12 м²`
...
Также нужно учесть индуктивное сопротивление (завихрения воздуха на концах крыльев). Этот коэффициент прибавляется к коэффициенту лобового сопротивления, а зависит зависит от удлинения крыла и угла атаки.
...
Вычисляем (угол атаки 5°):`0.4/(1.4*6.4)=0.045`  
Вычисляем (угол атаки 9°):`0.8/(1.4*6.4)=0.09`  
Вычисляем (угол атаки 15°):`1.2/(1.4*6.4)=0.14`
...
Пегасы и кирпичи — друзья на века.  
...
Вычисляем (угол атаки 15°, 70 км/час, 0.37 м² проекция):  
```
0.64*((1.2*(70/3.6)^2)/2)*0.37=54 ньютона (5.5 кгс)
```

Лобовое сопротивление пегаса на взлёте весьма невелико. Возвращаясь к расчётам реактивной тяги, на одном цикле движка (5 кг рабочего вещества со скоростью выброса 437 м/с) пегас может удерживать скорость и подниматься в течении: `5/(54/437)=40 секунд`

Вычисляем (угол атаки 9°, 100 км/час, 0.24 м² проекция):  
```
...
Вычисляем (угол атаки 0°, 200 км/час, 0.24 м² проекция):  
```
...
Короткое и широкое крыло, это плохо. Аэродинамические качества его ужасны при любых углах атаки, хотя и существует множество хитростей для управления пограничных слоем, применяемых на современных экранолётах.
...
Вычисляем (угол атаки 15°, 70 км/час, 4.16 м² проекция):  
```
...
Вычисляем (угол атаки 0°, 50 км/час, 2 м² проекция):  
```
0.04*((1.2*(70/3.6)^2)/2)*2=9.3 ньютонов (1 кгс)
```

Горизонтальный полёт на контуре лёгок и приятен. На самом деле это втрое лучше, чем лететь на собственных крыльях. На одном цикле двигателя можно пройти: `5/(9.3/437)=235 секунд` или 4.6 километров. Это та скорость с которой пегас может лететь и вместе с тем накапливать энергию. Высота небольшая, силу притяжения целиком компенсирует магнитная левитация, да и шума от двигателей практически нет.

Вычисляем (угол атаки 0°, 200 км/час, 2 м² проекция):  
```
...
Килоджоуля в кубометре недостаточно даже чтобы нагреть воздух на один градус, но пегасам хватает того, что есть. Как, впрочем и остальным. Магия в мире пони, это магнитная левитация, высокопрочные мембраны, рекомбинация материалов и молекулярные фильтры. Энергия в воздухе легко поддаётся концентрации и передаче. Например, средняя единорожка может за несколько секунд, выбросить из рога мембрану с радиусом в 4 метра. Объём этой полусферы будет равен:`4*(3.14159265*4^3)/3/2=134 м³`, а площадь поверхности:`4*3.14159265*4^2/2=100 м²`. С толщиной мембраны в микрометр она использует всего 17 грамм вещества, а соберёт 134 КДж энергии. Эта мембрана может обхватить любой предмет: может взорваться с мощностью в 32 грамма тротила (почти как граната АГС), а может летать несколько минут на магнитном диске и реактивной тяге. Понячья левитация, как она есть.
...
Пегасьи накопители энергии, это их огромные маховые перья. В среднем оперение пегаса весит около трёх килограмм (4% от массы тела), в каждом килограмме перьев пять грамм сверхпроводящих микрочастиц, так что плотность энергии 15 мегаджоулей на килограмм оперения. 45 Мегаджоулей энергии на каждого пегаса.

Один цикл работы двигателя — 1.3 мегаджоуля. Энергии хватит на 34 цикла в любом режиме, например: `34x16=544 секунду` или 9 минут полёта со скоростью в 200 км/час, или расстояние в 30 километров. То есть боевой радиус скоростного пегаса на собственных крыльях — 15 километров. Так он сумеет и долететь и вернуться, не задерживаясь на сбор энергии, весьма заметный и опасный в бою.
